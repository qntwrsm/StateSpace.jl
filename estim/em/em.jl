...
"""
function em(model::StateSpaceModel, fixed::NamedTuple, constraints::NamedTuple; 
            ϵ_abs::Real=1e-7, ϵ_rel::Real=1e-3, max_iter::Integer=1000)
    # Initialize state space model and system
    sys= init!(model, fixed, constraints)

    # Get dims
    (n,T)= size(model.y)
    p= lenght(sys.a1)
    n_params= number_parameters(model)
    ...
    abs_change= zero(eltype(ψ))
    rel_change= zero(eltype(ψ))
...
"""
    ecm!(model, fixed, constraints; ϵ_abs=1e-7, ϵ_rel=1e-3, max_iter=1000)
...
#### Returns
  - `ll::Real`          : log-likelihood value      
"""
function ecm!(model::StateSpaceModel, fixed::NamedTuple, constraints::NamedTuple; 
            ϵ_abs::Real=1e-7, ϵ_rel::Real=1e-3, max_iter::Integer=1000)
    # Initialize state space model and system
    sys= init!(model, fixed, constraints)
